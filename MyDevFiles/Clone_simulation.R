...
post_C <- sapply(ps_store, function(X) {
  sapply(X, function(XX) sapply(XX, function(post) post$marg[,"C"]))
}, simplify = F)
...
for(c in c_params){
  ...
  for(i in 1:n_repeats) {
    lines(y = post_C[[as.character(c)]][,i], x = n_markers,
          lwd = 1.5, col = cols_light[i], pch = 20)
    points(y = post_C[[as.character(c)]][,i], x = n_markers,
...
for(c in 100){ # Just focus on one since concentration parameter has little bearing
    ...
    for(m in n_markers){
      for(i in 1:n_repeats) {
        ...
        text(label = round(post_C[[as.character(c)]][as.character(m),i], 2), x = 0, y = 0, cex = 0.75)
      }
