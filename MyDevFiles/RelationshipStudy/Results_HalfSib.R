...
#
# Where are the log abline values from?
# ==============================================================================
rm(list = ls())
...
plot(NULL, xlim = c(1,max(n_markers)), ylim = c(0,2), bty = "n", las = 1,
     xaxt = "n", xlab = "Marker count", ylab = "All match to all diff. ratio")
...
# # Plot the data of repeat one
# ys_to_plot <- ys_store[["100"]][["admixture_TRUE"]]
# names(ys_to_plot) <- paste0("repeat", 1:n_repeats)
# plot_data(ys_to_plot[1], fs = fs_store[["100"]], marker_annotate = F)
