...
```
In this example, the most likely sequence is incompatible with the most likely relationship graph, and also the following four of five (is this due to the prior problem?): 

```{r}
ps <- sort.int(exp(sapply(post$RGs, function(RG) RG$logp)), 
               decreasing = T, index.return = T)
plot(ps$x[ps$x > 0]); abline(h = 5e-7, lty = "dashed")

# default genotype names
gs <- paste0("g", 1:6)
ts_per_gs <- c(1, 1, 1, 2, 2, 3)
par(mar = c(0.5, 0.5, 0.5, 0.5))
RGs_ids <- head(ps$ix, 6)
for(i in RGs_ids){
  RG <- post$RGs[[i]]
  plot_RG(RG_to_igraph(RG, gs, ts_per_gs), edge.curved=0.25, vertex.size=20)
}
```
Is this due to the prior problem? 
...
ys <- list(y1 = list(enroll = list(m1 = "A"), recur1 = list(m1 = "A")),
           y6 = list(enroll = list(m1 = "A"), recur1 = list(m1 = "A"), recur2 = list(m1 = NA), 
                     recur3 = list(m1 = NA), recur4 = list(m1 = NA), recur5 = list(m1 = NA), 
                     recur6 = list(m1 = NA)))
