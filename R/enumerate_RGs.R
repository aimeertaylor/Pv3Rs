...
#' constraints; see [vignette("enumerate")](../doc/enumerate.pdf). Each nested
#' set parition is an RG. Clone edges induce a cluster graph, equivalent to a
#' partition of genotypes, with no intra-episode clones allowed. Sibling edges
#' refine the clone partition, with no constraints (intra-episode siblings
#' allowed). Each nested set partition is encoded as a list. Each partition is
#' represented by a list of vectors (either `clone` or `sib`) and by a
#' membership vector (either `clone.vec` or `sib.vec`). By default, an RG
