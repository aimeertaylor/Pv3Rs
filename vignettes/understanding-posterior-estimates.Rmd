...
the maximal probability of recrudescence / reinfection depends on the following

- per-episode MOIs,
- episode count, 
- position of the recurrence in the sequence of episodes [finish and check colors],

and thus the 

- impact on how uncertainty is interpreted [re-write, explain bounds, referencing theoretical results]. 

Finally, we summarise results (for more details; see [development files
of the Pv3Rs source
...
However, we intuit 
- inter-marker dependence (linkage disequilibrium) will lead to overconfident posterior
probabilities, 
- population structure could lead to the misclassification of
reinfection as relapse / recrudescence, 
- population relatedness is encoded in allele frequency estimates locus-by-locus 
[Mehra et al. preprint 2024](https://www.biorxiv.org/content/10.1101/2024.04.16.588675v1)
and thus likely unimportant
...
## Homoallelic data without user-specified MOIs > 1
...
close to the prior but not equal to it because, for relationship graphs with
...
episode with data. Moreover, the lower bound on the cell count increases with the number
...
## Homoallelic data with user-specified MOIs > 1 and rare observations
...
- its output departs from the prior when data are homoallelic with user-specified MOIs > 1
and a rare observation.
...
Given equally likely recurrent states *a priori*, as the data increase with the
number of markers genotyped, posterior probabilities converge to either

- relapse with posterior probability one when the data suggest some parasites
across infections are regular siblings

- recrudescence with posterior probability less than one when the data are
compatible with recrudescence (less than one because data compatible with
recrudescence are also compatible with relapse)

- reinfection with posterior probability less than one when the data are
compatible with reinfection (less than one because data compatible with
reinfection are also compatible with relapse)

When the data are compatible with recrudescence and relapse (or reinfection and
relapse) the maximal posterior probability of recrudescence (or reinfection)
depends on the prior over relationships graphs. Otherwise stated, when two of
the three recurrent states are non-identifiable despite informative data, the
model returns a prior estimate based on the distribution over relationship
graphs.

For convenience, the prior over relationship is uniform over all graphs
compatible with a given recurrent state. As such, the maximal posterior
probability of recrudescence (or reinfection) depends on the number of
relationship graphs, which in turn depends on the number of genotypes per
episode and the number of episodes. Because the distribution over relationship
graphs is not invariant to different episode orderings, when there are two or
more recurrences, the position of a recurrence in the sequence of recurrences
has a bearing on posterior estimates.

Using rare matched and mismatched data on 100 markers, we demonstrate these
dependencies by increasing per-episode MOIs, which adds data inextricably, and
by increasing the number of recurrences without adding any data.

```{r}
marker_count <- 100 # Number of markers
ms <- paste0("m", 1:marker_count) # Marker names 
all_As <- as.list(sapply(ms, function(t) "A")) # As for all markers
all_Bs <- as.list(sapply(ms, function(t) "B")) # Bs for all markers
no_data <- as.list(sapply(ms, function(t) NA)) # NAs for all markers
```
...
The first recurrence in the sequence <span style="color: purple;">**O**ON</span>
(downward purple 1 triangle) has a higher probability of reinfection than the
...
The first recurrence in the sequence <span style="color: purple;">**O**ON</span>
(upward purple 1 triangle) has a lower probability of recrudescence than second
...
Unsurprisingly, the posterior of the first recurrence in the sequence N**O**O is
close to the prior (it has no preceding data), likewise for the second
recurrence in the sequence of <span style="color: purple;">OO**N**</span> (it
has no data). More surprisingly, for reasons explain below, the posterior of the
first recurrence of the sequence <span style="color: darkorange;">O**N**O</span> is
not close to the prior, despite having no data. On closer inspection, this is
not so surprising.

For match data, strong evidence for a clonal edge between episodes one and three
(<span style="color: darkorange;">ONO</span>, match) is incompatible with all
recurrence sequences ending with reinfection *and* reinfection followed by
recrudescence, pushing the second upward orange triangle to the simplex edge
*and* the first upward orange triangle away from the prior.

By comparison, strong evidence for a clonal edge between
episodes two and three (NO**O**, match) is incompatible with all sequences of
recurrences ending with reinfection, pushing the second upward green triangle
away from the prior, but not the first upward green triangle; likewise, strong evidence
for a clonal edge between episodes one and two (OON, match) is incompatible with
all sequences of recurrences starting with reinfection, pushing the first upward
purple triangle away from the prior but not the second.
...
On close inspection the upward orange 1 triangle makes sense: if you know a
first recurrence is followed by a second MOI = 1 recurrence that is a clone of
the MOI = 1 enrolment episode, it is very unlikely the second recurrence is a
clone drawn from a new mosquito as it would be if it were a recrudescence of a
reinfection. As such, the data on the enrolment episode and the second
recurrence tell us something about the first recurrence, and thus the posterior
estimate of the first recurrence deviates from the prior even though the first
recurrence has no data.

The explanation for mismatch data in the sequence ONO is similar as that for
match data: strong evidence for a stranger edge between episodes one and three
is incompatible with a double recrudescence, so the posterior of the first
recurrence (downward orange 1 triangle) deviates from the prior despite the
first recurrence having no data.
...
# Interpreting uncertainty [re-do]

Non-certain posterior estimates can be uncertain for two not mutually exclusive
inclusive reasons,

- more data are needed 
- the states are not identifiable 

To tease these two factors apart, we could report relative uncertainty? e.g.,
the posterior estimate relative to the posterior approximation of the maximal
probability.

[re-write] The uniform prior over relationship graphs compatible with a given sequence of
...
perturbation to the ratios of observations (all alleles match across the three
half siblings etc.) can lead to a large deviation in the odds of relapse versus
reinfection. The purple, red, and green trajectories below have higher than the
expected 0.5 intra-to-inter match ratios; moreover, the purple trajectory's
intra-to-inter match ratio consistently exceeds $0.5\times log_2(2/5)*  — a
condition found theoretically to concentrate posterior probability on
reinfection under certain conditions. <!-- 2^(2m3) >> 2^(m-2m2) -->
