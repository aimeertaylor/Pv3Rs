...
demonstrate this dependency by increasing per-episode MOIs and by increasing the
number of recurrences. Recurrences can be added without adding data, thereby
...
all_ACs <- sapply(ms, function(t) c("A", "C"), simplify = F) # As for all markers
...
The data do not change the maxima:

```{r}
sm(compute_posterior(y = list(enrol = all_As, recur = all_As), fs, MOIs = c(2,1))$marg)
sm(compute_posterior(y = list(enrol = all_ACs, recur = all_As), fs)$marg)
sm(compute_posterior(y = list(enrol = all_As, recur = all_Bs), fs, MOIs = c(2,1))$marg)
sm(compute_posterior(y = list(enrol = all_ACs, recur = all_Bs), fs)$marg)
```
