...
relationship graphs; see XXX. Otherwise stated, when two of three states for a
given recurrence are non-identifiable despite ample data, the model returns a
probability that can be expressed as a function of the prior over recurrent
state sequences and the prior over relationship graphs.
...
The maximum probability of recrudescence decreases with the imbalance between
the MOI at enrolment and the MOI of recurrence increases with the number of possibly independent inter-episode edges. [Check always case XXX]
...
Meanwhile, the graph likelihoods change: 
...
of possibly independent inter-episode edges, and thus with additional
intra-episode genotypes regardless of their nature (intra-episode strangers,
...
When user specified MOI estimates are more diverse than the analaysed data (the
data used for recurrent state inference), Pv3Rs sums over graphs with cliques of
three or more intra-episode MOIs (there is nothing in the analysed data to
suggest otherwise); when the analysed data are as diverse as the MOIs, graphs
with cliques of three or more intra-sibling parasites have zero likelihood. In
either case, the true number of siblings per group collapses to two per group,
capping elevated maximum probabilities, 

For example, let's write a function to simulate an enrolment episode comprising
...
simulate_data <- function(n_alleles){
  ...
  n_markers <- 100 # Number of markers
  ...
  fs <- sapply(markers, function(m) {
    sort(setNames(MCMCpack::rdirichlet(1, rep(c_param, n_alleles)), 
                  alleles), decreasing = T)
  ...
  # Construct children from parental allocations (streamline XXX; and cs above)
...
However, MOI estimates based on maximum per-marker allele counts are at most three and one when markers are highly multiallelic:
...
```{r echo = FALSE}
post <- sm(compute_posterior(y = multiallelic$data, 
                             fs = multiallelic$fs, return.logp=T))
llikes <- sapply(post$RGs, function(RG) RG$logp) # Extract log likelihoods
MOIs <- c(3,1) #determine_MOIs(data)
gs <- paste0("g", 1:sum(MOIs))
ts_per_gs <- rep(1:length(MOIs), MOIs)
par(mar = c(0.5, 0.5, 0.5, 0.5))
par(mfrow = c(3,2))
for(RG in post$RGs[which(is.infinite(llikes))]) {
...
have zero likelihood, they contribute to maximum probabilities of
recrudescence / reinfection through the conditionally uniform prior on graphs.
However, the contribution is generally very small (less than 2.5\%). % See (import data? XXX) 
