...
Because the prior on a uniformly distributed graph is a function of the number
of graphs, and that graph count depends on the number of genotypes within and
across episodes, the maximum probability of recrudescence / reinfection depends
on per-episode MOIs and episode counts, which vary across trial participants.
Using rare matched and mismatched data on 100 markers, we demonstrate this
dependency by increasing per-episode MOIs, which adds data inextricably, and by
increasing the number of recurrences. Recurrences can be added without adding
data, thereby demonstrating the impact on the maximum probability of
recrudescence / reinfection of the prior on graphs in isolation, i.e., without
