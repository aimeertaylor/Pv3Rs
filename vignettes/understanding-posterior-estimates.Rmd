...
n_alleles <- 2 # Number of alleles per marker (marker cardinality) # XXX Change to illustrate case
...
# Sample parental allocations dependently (generates meiotic siblings) per call
cs1 <- recombine_parent_ids(chrs_per_marker)
cs2 <- recombine_parent_ids(chrs_per_marker) 

# Construct children from parental allocations (streamline XXX; and cs above)
children1 <- sapply(1:n_markers, function(i) {
  sapply(1:ncol(cs), function(j) parents[i,cs[i,j]])
})
children2 <- sapply(1:n_markers, function(i) {
  sapply(1:ncol(cs), function(j) parents[i,cs[i,j]])
})
colnames(children1) <- markers
colnames(children2) <- markers

# Make parasite infections
initial <- apply(rbind(children1[1:2,], 
                       children2[1:2,], 
                       strangers[,3]), 2, unique, simplify = F)

# Add one highly diverse marker 
data <- list(enrol = initial, recur = as.list(children1[1,]))
post <- sm(compute_posterior(y = data, MOIs = c(3,1), fs, return.logp=T))
llikes <- sapply(post$RGs, function(RG) RG$logp) # Extract log likelihoods
