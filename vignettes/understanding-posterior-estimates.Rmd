...
- per-episode MOIs,
- episode count, 
- position of the recurrence in the sequence of episodes,
...
compatible with recrudescence (because data compatible with a recrudescence is
also compatible with relapse)

- reinfection with posterior probability less than one when the data are
compatible with reinfection (because data compatible with a reinfection is also
...
depends on the the prior over relationships graphs. The prior over relationship
is uniform over all graphs compatible with a given recurrent state, and thus on
the number of relationship graphs. The total number of graphs depends on the
number of genotypes per episode and the number of episodes. 
...
(downward purple 1 triangle) despite both recurrences having the same
...
(ONO, match) is incompatible with sequences of recurrences ending with
reinfection *and* reinfection followed by recrudescence, pushing the first
upward orange triangle away from the prior and second upward orange triangle to
the simplex edge. Meanwhile, strong evidence for a clonal edge between episodes
two and three (NOO, match) is incompatible sequences of recurrences ending with
reinfection, pushing the second upward green triangle away from the prior but
not the first upward green triangle; and strong evidence for a clonal edge
between episodes one and two (OON, match) is incompatible with sequences of
recurrences starting with reinfection, pushing the first recurrence (first
upward purple triangle) away from the prior but not the second.
...
evidence for a stranger edge between episodes two and three (NOO, green) is
incompatible with sequences ending with recrudescence; and strong evidence for a
stranger edge between episodes one and two (OON, purple) is incompatible with
sequences starting with recrudescence.
