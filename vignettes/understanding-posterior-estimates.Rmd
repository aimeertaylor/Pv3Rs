...
The maximum probability of recrudescence decreases with the imbalance between the MOI at
enrolment and the MOI of recurrence increases with the number of
inter-episode edges but. [XXX]

To increase per-episode MOIs, we can do as above and increase user-specified
MOIs explicitly, which amplifies the data implicitly, or we can increase the
polyclonality of the data explicitly, s.t. MOIs modelled under Pv3Rs increase
implicitly. Either way, we get the same maximum probabilities:
...
Meanwhile, the graph likelihoods change. For example, consider a MOI = 2 enrolment
episode followed by a MOI = 1 recurrence: for monoclonal enrolment data with a
user specified MOI of 2, the likelihood is only appreciable for relationship
graphs where all intra-episode relationships are sibling and the inter-episode
relationships are clonal. Whereas for polyclonal data, the likelihood is only
appreciable for relationship graphs where all intra-episode relationships are
stranger and the inter-episode relationships are clonal.
...
# Plot log likelihoods by graph classification
plot(llikes_sorted$x, pch = 20, col = "lightgray", bty = "n",
...
legend("bottomright", pch = 20, bty= "n", cex = 0.75,
       ...
       legend = c("Graphs with intra-episode strangers and inter-episode clones",
                  "Graphs with intra-episode strangers and inter-episode clones and siblings",
                  "All other graphs"))
title(main = sprintf("Recrudescence with probability %s", 
                     round(post$marg[,"C"],4)))
```
...
```{r echo = FALSE}
# ------------------------------------------------------------------------------
# Maximum likelihood graphs for illustrative case with MOIs 2 & 1
# ------------------------------------------------------------------------------
# Classify graphs by state
MOIs <- c(2,1)
gd <- get_graph_dist(MOIs)
