...
do.call(rbind, lapply(ys, function(y) suppressMessages(compute_posterior(y, fs))$marg)) 


# MOI increases with external input
MOIs <- list(c(2,1), c(2,2), c(3,2)) 
y <- list(enroll = list(m1 = c('A','B')), recur = list(m1 = NA)) # Data 
do.call(rbind, lapply(MOIs, function(x)suppressMessages(compute_posterior(y,fs,MOIs=x))$marg)) 
```

## Rare homoallelic data with user-specified MOIs > 1: prior departure 
...
posteriors_ext <- rbind(suppressMessages(compute_posterior(match, fs_rare))$marg, 
                        suppressMessages(compute_posterior(match, fs_comn))$marg, 
                        suppressMessages(compute_posterior(mismatch, fs_comn))$marg,
                        suppressMessages(compute_posterior(parmatch, fs_comn))$marg,
                        suppressMessages(compute_posterior(parmatch, fs_rare))$marg)

posteriors_new <- suppressMessages(compute_posterior(match_mismatch, fs_rare))$marg
...
the prior on the recurrent state sequences and the prior on the relationship
graphs; see [vignette("graph-prior")](graph-prior.pdf).
...
hom_match_data <- list(enrol = all_As, MOIs11_match_data = all_As)
het_match_data <- list(enrol = all_ACDs, MOIs31_match_data = all_ACs)
hom_mismatch_data <- list(enrol = all_As, MOIs11_mismatch_data = all_Bs)
het_mismatch_data <- list(enrol = all_ACDs, MOIs31_mismatch_data = all_BEs)
...
the number of intra-episode siblings.  Arguably, maximum probabilities should
not increase with more than two intra-episode siblings because siblings are not
independent. In practice, when MOIs under the Pv3Rs model are based on estimates
from bulk (not single-cell) parasite genetic data, groups of three or more
siblings are modelled as groups of two; see
[vignette("intra-episode-siblings")](intra-episode-siblings.html).
...
see [vignette("graph-prior")](graph-prior.pdf).
