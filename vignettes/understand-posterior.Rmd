...
The maximum probabilities increase with balanced per-episode MOIs (see XXX) and
thus with the number of possibly independent inter-episode edges (multiple lines
of independent evidence strengthen a case). 
...
over graphs with cliques of three or more intra-episode MOIs (there is nothing
...
per-marker alleles). We recover the same probability of recrudescence using both
the
...
because we simulate data for a large number of markers (200):
