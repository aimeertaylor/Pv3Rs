---
title: "Understand posterior estimates"
output: rmarkdown::html_vignette
  toc: true
vignette: >
  %\VignetteIndexEntry{Understand posterior estimates}
...
al. 2025](https://doi.org/10.1093/g3journal/jkaf018).
...
When data on a second marker are added and the second observation is consistent
with the first the strength of inference generally increases (see arrows in or
entering regions of posterior probability greater than 0.5). An exception being
the addition of a common match to a common partial match, which is not very
informative (central arrow). When a mismatch is added to a rare match (diamond),
relapse becomes the most probable state. This demonstrates the requirement of
...
those compatible with sequences of relapse, bounds induced by the prior on
the posterior can be computed *a priori* as a function of the prior on the
recurrent state sequences and the prior on the relationship graphs; see
[vignette("understand-graph-prior")](understand-graph-prior.pdf).

Because the probability mass function of a uniformly distributed discrete random
variable depends on the size of its support, the prior on a uniformly
distributed graph (and thus the bounds induced on the posterior) depends
on the size of the graph space, which depends on the size of the graphs within
the graph space (for a given space, all graphs are the same size), which depends
on per-episode MOIs and episode count, which varies across trial participants. 

Using rare matched and mismatched data on 100 markers, we demonstrate the
dependency on graph size of the upper posterior bounds induced by the prior by
increasing per-episode MOIs, which adds data inextricably; by increasing the
number of recurrences without adding data, we demonstrate the dependency of the
upper posterior bounds induced by the prior *plus* the absence of data on all
but the first recurrence.
...
```{r}
MOIs <- list(c(1,1), c(2,2), c(3,3), c(2,1), c(3,1), c(3,2)) 
y_match <- list(enrol = all_As, recur = all_As)
y_mismatch <- list(enrol = all_As, recur = all_Bs)
Recur_prob_correlate <- sapply(MOIs, function(x) x[2]/(0.5 + (x[1] - x[2])))
```
...
# Add legends
legend("left", pch = c(1, 20), pt.cex = c(2, 1), bty = "n", inset = 0,
       legend = c("Maximum posterior probability", "Posterior estimate"))
legend("right", pch = 20, pt.cex = 2, bty = "n", inset = 0.2,col=1:length(MOIs), 
       legend = gsub("c", "", as.character(MOIs)), title = "MOIs") # legend
```

The posterior probabilities of recrudescence increase with increasing graph size
when the MOIs are balanced. The posterior probabilities of reinfection increase
with increasing graph size.


### Aside: counts of intra-episode siblings 

To increase per-episode MOIs, we can do as above and increase user-specified
MOIs without changing the data, or we can increase the polyclonality of the
data, s.t. MOIs modelled under Pv3Rs increase. Either way, we get the same
...
MOI32_match_data <- list(enrol = all_ACDs, MOIs31_match_data = all_ACs)
MOI11_mismatch_data <- list(enrol = all_As, MOIs11_mismatch_data = all_Bs)
MOI32_mismatch_data <- list(enrol = all_ACDs, MOIs31_mismatch_data = all_BEs)

rbind(sm(compute_posterior(MOI11_match_data, fs, MOIs = c(3,2))$marg), 
      sm(compute_posterior(MOI32_match_data, fs)$marg), 
      sm(compute_posterior(MOI11_mismatch_data, fs, MOIs = c(3,2))$marg),
      sm(compute_posterior(MOI32_mismatch_data, fs)$marg))
```
...
y <- MOI11_match_data
post <- sm(compute_posterior(y, fs, MOIs = MOIs, return.logp=T))
llikes <- sapply(post$RGs, function(RG) RG$logp) # Extract log likelihoods
llikes_sorted <- sort.int(llikes, index.return = T) # Sort log likelihoods
...
legend("topleft", pch = 20, bty= "n", cex = 0.75, col = c("black", "lightgray"), 
       legend = c("Graphs with intra-episode siblings and inter-episode clones",
                  "All other graphs"))
title(main = sprintf("Recrudescence with probability %s", 
                     round(post$marg[,"C"],4)))

# Get graph log likelihoods for polyclonal data without user-specified MOIs
y <- MOI32_match_data
post <- sm(compute_posterior(y, fs, return.logp=T))
llikes <- sapply(post$RGs, function(RG) RG$logp) # Extract log likelihoods
llikes_sorted <- sort.int(llikes, index.return = T) # Sort log likelihoods
...
legend("topleft", pch = 20, bty= "n", cex = 0.75, col = c("black", "lightgray"), 
       legend = c("Graphs with intra-episode strangers and inter-episode clones",
                  "All other graphs"))
title(main = sprintf("Recrudescence with probability %s", round(post$marg[,"C"],4)))
```

For the monoclonal data with elevated user-specified MOIs, all intra-episode
parasites in maximum likelihood graphs are siblings. For the polyclonal data,
all intra-episode parasites in maximum likelihood graphs are strangers. Both
have the same maximum probabilities of recrudescence.

One upshot of this is that maximum probabilities can be increased by increasing
the number of intra-episode genotypes regardless of their nature (intra-episode
strangers, which are independent, versus intra-episode siblings, which are
dependent). Arguably, maximum probabilities should not increase with more than
two intra-episode siblings (more lines of evidence strengthen a case, but only
if they are independent). In practice, Pv3Rs behaves accordingly: groups of
three or more siblings are modelled as groups of two because modelled MOIs are
based on estimates from bulk (not single-cell) parasite genetic data (XXX: move
section below to another vignette and cite that vignetter here).
...
When user specified MOI estimates are more diverse than the data, Pv3Rs sums
over graphs with cliques of three or more intra-episode siblings (there is nothing
in the data to suggest otherwise); when the analysed data are as diverse as the
MOIs, graphs with cliques of three or more intra-sibling parasites have zero
likelihood. In either case, the true number of siblings per group collapses to
two per group, capping elevated maximum probabilities; see following example.

First, let's write a function to simulate an enrolment episode comprising
a stranger plus a group of four siblings, two from one oocyst, two from another,
all drawing from the same two unrelated parental genotypes, and a recurrence
with one sibling. Technically, the enrolment episode contains five genetically
distinct genotypes and thus has a MOI of five.
...
The marginal probability of that the first recurrence is a recrudescence /
...
legend("left", pch = c(1, 20), pt.cex = c(2, 1), bty = "n", inset = 0, 
       legend = c(
         "Maximum probability induced 
by the prior plus knowledge
that all but the first 
recurrence has data", 
         "Posterior estimate"))
...
ys100 <- list("1_recurrence" = list(enroll = A100,
                                    recur1 = A100),
              "2_recurrence" = list(enroll = A100,
                                    recur1 = A100,
                                    #recur2 = NA100,
                                    recur3 = NA100),
              "3_recurrence" = list(enroll = A100,
                                    recur1 = A100,
                                    recur2 = NA100,
                                    #recur3 = NA100,
                                    #recur4 = NA100,
...
# Compute posterior
p1 <- sapply(ys1, function(y) sm(compute_posterior(y = y, fs))$marg["recur1",])
p2 <- sapply(ys2, function(y) sm(compute_posterior(y = y, fs))$marg["recur1",])
p5 <- sapply(ys5, function(y) sm(compute_posterior(y = y, fs))$marg["recur1",])
p100 <- sapply(ys100, function(y) sm(compute_posterior(y = y, fs))$marg["recur1",])
...
# For seqs CC, CI, CL, LC, LI, LL
# there are 1, 1,  3,  3,  3,  12 compatible graphs 
# there are 1, 1,  3,  1,  1,  3 compatible graphs with first pair as clones
# when MOIs are 1, 1, 1

# CC, CL, CI should be equal 
post <- sm(compute_posterior(y = list(enroll = all_As, 
                                      recur1 = all_As,
                                      recur2 = no_data), fs = fs))
c("Estimate" =  post$marg["recur1","C"], 
  "Maximum" = 3 / (3 + 1/3 + 1/3 + 3/12)) # Relies on knowledge of data (eq 4)
...
The prior probability distribution over the first two genotypes changes with the
addition of recurrences due to the uniformity of the prior over graphs
...
a posterior estimate is from its maximum, which can be computed *a priori* when 
there is only one recurrence; [vignette("understand-maxima")](understand-maxima.pdf). 
...
classify probable reinfection assuming recurrence states are equally likely 
*a priori*, we can discount *a priori* all trail participants with a single
...
[vignette("understand-half-siblings")](understand-half-siblings.pdf). The
purple, red, and green trajectories below have higher than the expected 0.5
intra-to-inter match ratios; moreover, the purple trajectory's intra-to-inter
match ratio consistently exceeds $0.5\times \text{log}_2(\dfrac{2}{5})$, a
condition found theoretically to concentrate posterior probability on
reinfection under certain conditions; again, see [vignette("understand-half-siblings")](understand-half-siblings.pdf). 
