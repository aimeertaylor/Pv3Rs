...
compatible with relapse. Otherwise stated, recrudescence and reinfection are not
fully identifiable given genetic data alone.
...
relationship graphs; see 
[vignette("understand-maxima")](understand-maxima.pdf). 
...
distributed graph, and thus the maximum probability of recrudescence /
reinfection, depends on the size of the graph space, which depends on the size
of the graphs within the graph space (for a given space, all graphs are the same
size), which depends on per-episode MOIs and episode counts, which vary across
...
with recrudescence (match)
and reinfection (mismatch)", 
"Posterior probability"))
...
The maximum probabilities of reinfection increase with the size of the graph 
(see Table 1 of [vignette("understand-maxima")](understand-maxima.pdf)). 

[reword XXX and thus with the number of possibly independent inter-episode edges (multiple lines of independent evidence strengthen a case).]
...
This means that maximum probabilities can be increased by increasing the number
of intra-episode genotypes regardless of their nature (intra-episode strangers,
which are independent, versus intra-episode siblings, which are are dependent).
Arguably, maximum probabilities should not increase with more than two
intra-episode siblings (more lines of evidence strengthen a case, but only if
they are independent). In practice, Pv3Rs behaves accordingly: groups of three
or more siblings are modelled as groups of two because modelled MOIs are based
on estimates from bulk (not single-cell) parasite genetic data:
...
legend("left", pch = c(1, 20), pt.cex = c(2, 1), bty = "n", inset = 0, 
       legend = c(
         "Maximum probability based on prior 
         over graphs compatible with 
         reinfection first (mismatch), 
         recrudescence first (match)", "Posterior probability"))
...
Checking maxima when recurrences have data.. 
...
post <- sm(compute_posterior(y = list(enroll = all_As, 
                             ...
                             fs = fs))
c("Estimate" = as.numeric(post$joint["CL"]),
  "Maximum" = 1 / (1 + 3/12))
```
