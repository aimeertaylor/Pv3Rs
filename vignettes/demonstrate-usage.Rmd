...
all permutations because the scheme is too complicated. Likewise, all
permutations are summed over when `return.logp = TRUE`, rendering the likelihood
of every graph within an equivalence class equal and not dependent on the
internal scheme used when symmetry is exploited.
...
current example, the relationship graphs with the largest likelihood (e.g.,
...
below), and that graphs in the equivalence class with the largest likelihood
might be incompatible with the most likely recurrent state sequence (e.g., here
the class with the largest likelihood contains graphs with sibling edges that
are incompatible with `r names(which.max(post$joint))`).
