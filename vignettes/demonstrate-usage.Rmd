...
**Aside** The most parsimonious MOI estimates compatible with the above data are
`r determine_MOIs(y, return.names = TRUE)` computed using `determine_MOIs()`. By
default, `compute_posterior` uses these MOI estimates. In this synthetic
example, markers are quart-allelic, imposing low upper bounds on MOI estimates
...
### 2) Compute posterior recurrence state posterior probabilities 

**Aside** We do not recommend running `compute_posterior()` for data whose total
genotype count (sum of MOIs) exceeds eight. That said, we have not encoded a
hard limit. In our experience, it is possible, but very long, to generate
estimates using data with a total genotype count up to 10; above 10 genotypes,
...
Per-recurrence probabilities are computed by simply summing over probabilities
of a subset of states sequences:

```{r}
sprintf('post$marg for first-recurrence relapse, %s, is equal to %s, 
        the sum of probabilities over LC, LL and LI', 
        round(post$marg["Recurrence 1", "L"], 4), 
        round(post$joint["LC"] + post$joint["LL"] + post$joint["LI"], 4))
```
...
but computation was longer: To compute the posterior, summations over per-marker
allelic assignments that are equivalent up to within-episode genotype
...
compute time, especially when MOIs are large. Also, when user-specified MOIs are
greater than those given by `determine_MOIs()`, `compute_posterior()` sums over
all permutations because the representative-choosing scheme is too complicated.
 
The log-likelihood of each relationship graph is returned in the output. We plot
the relationship graph(s) with the largest likelihood. There are two graphs with
maximum likelihood; they are isomorphic up to within-episode genotype
permutations:
...
- The relationship graph(s) with the largest likelihood (e.g.,
those above) might not be in the equivalence class for which the data are most
probable when all relationship graphs in that class are summed over; see below. 

- The maximum-likelihood graph(s) might be incompatible with the
maximum-posterior state sequence, although they are compatible in this example;
recall the most-likely sequence is `r names(which.max(post$joint))`. 
