...
- how probability bounds could be used heuristically to assess marker informativeness 
...
permutations are summed over when `return.logp = TRUE`: to compute meaningful
graph likelihood values (i.e., values that are not dependent on the
aforementioned representative-choosing scheme) we need to sum over per-marker
allelic assignments, including those that are equivalent up to within-episode
...
<!-- see example in DevFiles/Investigate_Symmetries for an example --> 
...
the possibility that both are true without further -->
