...
This vignette, which is rather niche, expands a digression in 
...
write a function to simulate data on 200 markers with some specified allelic
richness (marker cardinality) for an enrolment episode comprising a stranger
plus a group of four siblings, two from one oocyst, two from another, all
drawing from the same two unrelated parental genotypes, and a recurrence with
one sibling. Technically, the enrolment episode contains five genetically
...
number of markers, we recover almost exactly the same posterior probabilities
using both the
...
necessarily less than 7 (recall that computationally feasible MOI vectors are
those where the total genotype count is at most eight).
