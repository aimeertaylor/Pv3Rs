...
three or more intra-episode siblings whenever the MOIs used by the Pv3Rs model
exceed two. However, these graphs only acquire non-zero likelihoods if the user
specifies MOIs that exceed the diversity in the data `y`.

In any case, when MOIs estimates are based on bulk parasite genetic
data (i.e., not single-cell data), groups of three or more
non-half intra-episode siblings collapse to groups of two, because:
...
- MOI estimates based on heteroallelic marker counts cannot exceed two for
groups of three or more non-half siblings because their heteroallelic marker
count cannot exceed that of the two parental genotypes from which they derive.
...
write a function to simulate data with specified allelic richness (marker
cardinality) for an enrolment episode comprising a stranger plus a group of four
siblings, two from one oocyst, two from another, all drawing from the same two
unrelated parental genotypes, and a recurrence with one sibling. Technically,
the enrolment episode contains five genetically distinct genotypes and thus has
a MOI of five.
...
(200 markers are hard coded in the `simulate_data` function above), we recover
almost exactly the same posterior probabilities using both the
...
However, given the polyallelic data the following graphs with cliques of three
intra-episode siblings have zero likelihood:
...
`r sapply(graphs_with_big_diff, gsub, pattern = "", replacement = " ")`) and 
thus those whose graph spaces include graphs with the largest cliques of
intra-episode siblings.

The contribution is likely even smaller for all vectors of three or more MOIs
that are computationally feasible under Pv3Rs because their constituent MOIs are
