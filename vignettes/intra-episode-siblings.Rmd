...
two. However, when the data input of `compute_posterior()` are as diverse as the
MOIs used by the Pv3Rs model, graphs with cliques of three or more intra-sibling
parasites have zero likelihood; when MOIs used by the Pv3Rs model are more
diverse than the data input of `compute_posterior()` (e.g., because the user
inputs elevated MOIs into `compute_posterior()`), graphs with cliques of three
or more intra-episode siblings have non-zero likelihoods. In either case,
provided MOIs used by the Pv3Rs model are based on estimates from per-episode
parasite genetic data generated and analysed in bulk (i.e., not single-cell
data), groups of three or more not-half intra-episode siblings collapse to
groups of two because
...
```{r}
ppost <- sm(compute_posterior(y = polyallelic$data, fs = polyallelic$fs, 
                              return.logp = T))
bpost <- sm(compute_posterior(y = biallelic$data, fs = biallelic$fs, 
...
```{r echo = FALSE, fig.height = 1.5, fig.cap="Graphs with zero likelihood"} 
...
points(y = maxima["I_with", condition], x = maxima["I_wout", condition], 
       pch = 21, bg = "red")
title(main = sprintf("Largest difference: %s", round(max(abs(diffs)), 4))) 
```
...
title(main = sprintf("Largest difference: %s", round(max_diff, 4))) 
...
Given the MOI vectors for sequences of multiple recurrences that are
computationally feasible under Pv3Rs have lower MOIs due to the constraint on
the total number of genotypes (recall that we do not recommend running
`compute_posterior()` when the total genotype count exceeds eight) we surmise
the contribution is even smaller for all other MOI vectors; indeed, it is when
we explore the bounds induced by the prior plus knowledge of no data on all but
the first recurrence:

```{r}
condition <- c(maxMOIs > 2 & nMOIs > 2)
range(maxima["I_with", condition] - maxima["I_wout", condition])
range(diffs <- maxima["C_with", condition] - maxima["C_wout", condition])
```
