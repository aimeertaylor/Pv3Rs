---
title: "Understanding posterior output"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Understanding posterior ouput}
...
In this vignette we document the output of `Pv3Rs::compute_posterior` when the
statistical model underpinning it is well specified and misspecified.

More specifically, when the model is well specified, we document the output of `Pv3Rs::compute_posterior` when the data are missing, limited, and ample, and as 
a function of genetic diversity (allele frequencies), recurrence counts and 
multiplicities of infection (MOIs). 

Thereafter, we document the output of `Pv3Rs::compute_posterior` when the model
is misspecified because of
...
Before starting, we define a helper function that gets relationship graphs (and
the recurrent state sequences they are compatible with) for a vector of
per-episode MOIs.

```{r}
get_graph_dist <- function(x) { # get RG vertex labels, RGs, sequences given RGs
...
y_missing = list(enrol = list(m1 = NA), recur = list(m1 = NA)) # Data
...
If the data are heteroallelic, the posterior is close but not equal to the prior
because, for relationship graphs with intra-episode siblings, the heteroallelic
data limits summation over identity-by-descent partitions to partitions with at
least two cells for the episode with data. The lower bound on the cell count
increases with the number of distinct alleles observed.
...
If the data are homoallelic, the observed allele is rare, and the MOI is greater
than one because of external input, the posterior departs from the prior because
rare intra-episode allelic repeats limit the likelihood to summation over
relationship graphs with intra-episode relatedness. We can use different MOIs to
demonstrate this: changing the MOIs changes the distribution over relationship
graphs, and thus the posterior output.

```{r}
#-------------------------------------------------------------------------------
# Compute posterior for homoallelic data on only one episode with MOI > 1
#-------------------------------------------------------------------------------
MOIs <- list(c(2,1), c(2,2), c(3,2)) # Different MOIs 
...
in the first episode are siblings (note the log scale).
...
of `compute_posterior` depends on the whether the *observation* is a match or a 
mismatch, and the frequencies of the observed alleles. For example, 
...
possible *a posteriori* and the posterior is close to the prior.
...
text(x = xy["x", ], y = xy["y", ], pos = 3, cex = 0.75, # Annotate
     labels = c("common match", "rare match", "mismatch", 
                "partial common match")) 
```

Given equal probabilities of relapse, recrudescence and reinfection *a priori*, 
posterior probabilities converge to either
...
results_match <- sapply(MOIs, function(x) 
  suppressMessages(compute_posterior(y = ys_match[["1_recurrence"]], 
                                     fs, MOIs = x)$marg))
results_mismatch <- sapply(MOIs, function(x) 
  suppressMessages(compute_posterior(y = ys_mismatch[["1_recurrence"]], 
                                     fs, MOIs = x)$marg))
...
The likelihood is only appreciable for relationship graphs where the first
episode parasites are all siblings and the intrer-episode edges are clones (note
the log scale).
...
points(y = llikes_sorted$x[(RGs_not_I & RGs_edges_within)[llikes_sorted$ix]], 
       ...
       col = "cornflowerblue", pch = 20, cex  = 0.5)
...
legend("bottomright", pch = 20, bty= "n", 
       col = c("black", "cornflowerblue", "lightgray"), 
       legend = c("Graphs with only siblings in the first episode and inter-episode clones",
                  "Graphs with only siblings in the first and inter-episode clones and siblings",
                  "All other graphs"))
```
...
recurrent states is **NOT** uninformative. As such, when recurrent states are
...
the per-episode MOIs (Table XXX). Otherwise stated, the posterior recurrent
state space is a function of the prior on the recurrent states, the number of
episodes and their MOIs (Figure XXX).

Consequently, if we use a common threshold of say 0.8 to classify reinfection
based on genetic data alone (i.e., assuming recurrence states are equally likely
*a priori*), we can discount all individuals with a single monoclonal recurrence
following a monoclonal enrolment episode, because their posterior reinfection
probabilities will never exceed 0.75, even if their data are highly informative.
An non-uniform prior on recurrent states circumvents this issue: 
