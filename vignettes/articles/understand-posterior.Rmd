...
This vignette is a detailed sequel to `vignette(demonstrate-usage)`. In it, we
start by documenting the output of `compute_posterior()` when the statistical
...
- Data are limited to only [one marker](#one marker) and [two markers](#two markers)
...
- [Per-episode multiplicities of infections (MOIs)](#mois)
...
For more detailed results on misspecification can be found in the 
[development files of the Pv3Rs source package](https://github.com/aimeertaylor/Pv3Rs/tree/main/DevFiles/RelationshipStudy). 
...
space and time; household effects) will likely lead to the misclassification of
