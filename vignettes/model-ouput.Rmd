...
```{r}
# Define a function to get gs, RGs, recurrence states given RGs, and thus avoid repetition
...
the prior because rare intra-episode allelic repeats limit summation over
relationship graphs to relationship graphs with intra-episode relatedness.

```{r, cache = TRUE}
#-------------------------------------------------------------------------------
# Generate input for homoallelic data on only one episode 
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
MOIs <- list(c(2,1), c(3,1), c(2,2), c(3,2), c(3,3)) 
...
legend("right", col = 1:length(MOIs), pch = 20, title = "MOIs", inset = 0,
...
When data on a single marker across multiple episodes are comparable, the output
...
the distribution over relationship graphs, where relationship graphs compatible
with a given state are considered equally likely *a priori*. 
...
Before demonstrating the impact of the prior distribution over relationship
graphs, we define a helper function. For a given set of MOIs, the helper
function defines the graph distribution by the relationship graphs and their
compatibilities with differ states.
...
# Add legends
legend("bottom", pch = c(20, 1), pt.cex = c(1, 2), bty = "n", 
