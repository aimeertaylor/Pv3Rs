...
```{r}
y <- list("Enrollment" = list(m1=c('C','G','T'), m2=c('A','C'), m3=c('C','G','T')),
          "Recurrence 1" = list(m1=c('C','T'), m2=c('A'), m3=c('A','C')),
          "Recurrence 2" = list(m1=c('T'), m2=c('A'), m3=c('A')))
...
plot_data(ys = list(example = y), fs = fs)
```


Given the number of alleles observed during each episode, the most parsimonious explanation is that there are three, two, and one distinct parasite genotypes* across the three episodes respectively. 
(*A genotype is defined here as a realisation of the parasite genome, which is haploid and has 14 chromosomes.)

```{r}
determine_MOIs(y = y)
```

If more precise MOI estimates based on alternative data are available, they can be input into the function `compute_posterior()` that computes the posterior probability of recurrence states. 

When performing inference on the recurrence states, the bulk of the computational time lies in computing the log-likelihood of each possible relationship graph over the six genotypes.
...
The marginal posterior probabilities can be plotted on the simplex using the function `plot_simplex()`. 
...
plot_simplex(v_labels = c(C = "Recrudescence", 
                          L = "Relapse", 
                          I = "Reinfection")[colnames(post$marg)], classifcation_threshold = 0.5)
...
```{r}
# Extract the most likely relationship graph
RG <- post$RGs[[which.max(sapply(post$RGs, function(RG) RG$logp))]]

gs <- paste0("g", 1:6) # Generate genotype names
ts_per_gs <- rep(1:length(y), determine_MOIs(y)) # episode index per genotypes
...
# The sequences of state compatible with the most likely relationship graph
seqs_comp_MLE_RG <- compatible_rstrs(RG, split(gs, ts_per_gs))
```

**N.B.** This approach of finding the `most likely' relationship graph does not take into account of symmetries. Specifically, relationship graphs that are isomorphic up to within-episode genotype permutations have the same likelihood. The user should note that the most likely equivalence class of relationship graphs may not contain the relationship graph with the largest likelihood. 

The figure below shows histograms of the likelihoods of the relationship graphs compatible with each possible sequence of recurrence states, along with the likelihood of the sequence of recurrence states (red vertical line). Although sequences `r seqs_comp_MLE_RG` are all associated with the relationship graph with the largest log-likelihood, we observe that the likelihoods of these sequences of recurrence states decrease as the number of compatible relationship graphs increase. This behaviour is expected due to our choice of a uniform prior over relationship graphs. 
...
```{r}
ys_suitable <- y.dfs[suitable]
plot_data(ys = , fs = Fs_Combined)  
```
