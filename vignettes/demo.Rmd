...
realisation of the parasite genome, which is haploid and has 14 chromosomes.)

```{r}
determine_MOIs(y)
```

If more precise MOI estimates based on alternative data are available, they can
be used in genetic recurrent state inference by specifying the `MOI`
argument in the function `compute_posterior()`.

When performing genetic recurrent state inference, the bulk of the computational
time lies in computing log-likelihoods of relationship graphs (log p(Y|RG) over
genotypes.
